<div class="flex h-full w-full flex-1 flex-col gap-6 p-6">
    ...
    @if($this->requests->isEmpty())
        <div class="flex flex-1 items-center justify-center rounded-xl border border-zinc-200 py-20 text-center">
            <div>
                <flux:icon name="archive-box" class="mx-auto mb-3 size-10 text-zinc-300" />
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-zinc-100 bg-zinc-50 text-left">
                            ...
                            <th class="px-4 py-3 font-semibold text-zinc-600">{{ __('Travel by') }}</th>
                    ...
                    <tbody class="divide-y divide-zinc-100">
                        @foreach($this->requests as $request)
                            ...
                            <tr wire:key="request-{{ $request->id }}" class="transition-colors odd:bg-white even:bg-zinc-50/50 hover:bg-navy-50">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-2">
                                        @if($request->item_image_path)
                                            ...
                                            />
                                        @else
                                            <div class="flex size-10 items-center justify-center rounded bg-zinc-100">
                                                <flux:icon name="archive-box" class="size-5 text-zinc-400" />
                                            </div>
                                        @endif
                                ...
                                {{-- Poster --}}
                                <td class="px-4 py-3 text-zinc-700">{{ $request->user->name }}</td>
